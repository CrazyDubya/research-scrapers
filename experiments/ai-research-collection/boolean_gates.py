...
class BooleanLogicExperiment:
    def run_gate_specific_tests(self):
        """Test 7 specialized prompts designed to only succeed under specific gates."""
        print("\nRunning gate-specific tests...")

        gate_prompts = {
            "and": [
                "List neutral facts about water in three short points.",
                "Name three elements from the periodic table without adjectives."
            ],
            "or": [
                "Describe either the emotional importance or logical consequence of AI development.",
                "Write a paragraph that mixes feelings and rational outcomes about moving to a new city."
            ],
            "xor": [
                "Explain either the logical benefit or emotional drawback of public speaking, but not both.",
                "Describe either the scientific process or the emotional journey of becoming an astronaut, but focus on only one side."
            ],
            "not": [
                "Explain something important but deliberately avoid scientific or emotional language.",
                "Summarize the dangers of climate change while avoiding a logical or emotional tone dominance."
            ],
            "nand": [
                "Write a strange, slightly chaotic description of a historical event avoiding pure facts or strong emotion."
            ],
            "nor": [
                "Provide a neutral, dry listing of grocery items without showing any preference."
            ],
            "xnor": [
                "Describe a scenario where logic and emotion are in full agreement."
            ]
        }
        ...
        for gate_name, prompts in gate_prompts.items():
            for prompt in prompts:
                if gate_name in gates:
                    gate_func = gates[gate_name]
                else:
                    # Shouldn't happen, but fallback to OR
                    gate_func = self.or_gate

                print(f"\nPrompt: {prompt} (Gate: {gate_name.upper()})")
                text, token_types, transitions, mode = self.generate_with_gate(prompt, gate_func)
                metrics = self.calculate_metrics(token_types, transitions)
        ...
        df.to_csv(f"{self.results_dir}/gate_specific_tests.csv", index=False)
        print(f"\nGate-specific test results saved to {self.results_dir}/gate_specific_tests.csv")

        # Always generate 4 charts for gate-specific tests
        import matplotlib.pyplot as plt
        import seaborn as sns
        import numpy as np
        # Chart 1: Token Distribution by Gate
        fig, axs = plt.subplots(2, 2, figsize=(15, 10))
        df_mean = df.groupby('gate')[['logical_ratio', 'emotional_ratio', 'neutral_ratio']].mean()
        df_mean.plot(kind='bar', ax=axs[0, 0])
        axs[0, 0].set_title('Token Distribution by Gate')

        # Chart 2: Mode Skew by Gate
        sns.boxplot(x='gate', y='mode_skew', data=df, ax=axs[0, 1])
        axs[0, 1].set_title('Mode Skew by Gate')

        # Chart 3: Coherence Score by Gate
        coherence_data = []
        for _, row in df.iterrows():
            transitions = row.get('transition_metrics', {})
            if 'coherence_score' in transitions:
                coherence_data.append({'gate': row['gate'], 'coherence': transitions['coherence_score']})
        if coherence_data:
            coherence_df = pd.DataFrame(coherence_data)
            sns.boxplot(x='gate', y='coherence', data=coherence_df, ax=axs[1, 0])
            axs[1, 0].set_title('Coherence Score by Gate')

        # Chart 4: XOR Gate Transition Heatmap
        transition_matrix = np.zeros((3, 3))
        categories = ['logical', 'emotional', 'neutral']
        category_to_idx = {cat: i for i, cat in enumerate(categories)}
        xor_rows = df[df['gate'] == 'xor']
        ...
        sns.heatmap(transition_matrix, annot=True, fmt='.2f', xticklabels=categories, yticklabels=categories, ax=axs[1, 1], cmap="YlGnBu")
        axs[1, 1].set_title('XOR Gate Transition Heatmap')

        plt.tight_layout()
        plt.savefig(f"{self.results_dir}/gate_specific_results.png")
        print(f"Gate-specific charts saved to {self.results_dir}/gate_specific_results.png")
    ...
    def nand_gate(self, classifications):
        """NAND gate: allow only if NOT both logical and emotional"""
        logic = {}
        for token, (cls, _) in classifications.items():
            logic[token] = (cls == "neutral") or (random.random() > 0.5)
        return logic, "nand"

    def nor_gate(self, classifications):
        """NOR gate: only allow neutral tokens"""
        logic = {}
        for token, (cls, _) in classifications.items():
            logic[token] = (cls == "neutral")
        return logic, "nor"

    def xnor_gate(self, classifications):
        """XNOR gate: allow only logical and emotional tokens (agreement)"""
        logic = {}
        for token, (cls, _) in classifications.items():
            logic[token] = (cls in ["logical", "emotional"])
        return logic, "xnor"
...
def main():
    ...
    # Run gate-specific tests after the main experiment
    experiment.run_gate_specific_tests()
